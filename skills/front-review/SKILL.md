...
if [[ -f "$ARGUMENTS" ]]; then
  FILES_TO_REVIEW="$ARGUMENTS"
  echo "Reviewing specific file: $FILES_TO_REVIEW"

# If $ARGUMENTS is a commit hash
elif git cat-file -e "$ARGUMENTS^{commit}" 2>/dev/null; then
  FILES_TO_REVIEW=$(git diff-tree --no-commit-id --name-only -r "$ARGUMENTS")
  echo "Reviewing commit $ARGUMENTS"
  echo "Changed files: $FILES_TO_REVIEW"

# If $ARGUMENTS is --full or empty, compare with main branch
else
  MAIN_BRANCH=$(git symbolic-ref refs/remotes/origin/HEAD 2>/dev/null | sed 's@^refs/remotes/origin/@@' || echo "main")
  FILES_TO_REVIEW=$(git diff --name-only "$MAIN_BRANCH"...HEAD | grep -E '\.(tsx?|jsx?)$')
  echo "Reviewing all changed files on current branch vs $MAIN_BRANCH"
  echo "Files to review: $FILES_TO_REVIEW"
fi
```

**Execute the scope determination:**

- Files to review: !`ARGS="$ARGUMENTS"; if [[ -f "$ARGS" ]]; then echo "$ARGS"; elif git cat-file -e "$ARGS^{commit}" 2>/dev/null; then git diff-tree --no-commit-id --name-only -r "$ARGS" | grep -E '\.(tsx?|jsx?)$'; else MAIN_BRANCH=$(git symbolic-ref refs/remotes/origin/HEAD 2>/dev/null | sed 's@^refs/remotes/origin/@@' || echo "main"); git diff --name-only "$MAIN_BRANCH"...HEAD | grep -E '\.(tsx?|jsx?)$'; fi`
